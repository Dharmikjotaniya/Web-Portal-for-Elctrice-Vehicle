...
async function ViewChargingStationSlots(req, res) {
    try {
        ...
        const slots = await collection.find({
            charging_station_id: new ObjectId(stationId)
        }).toArray();

        return res.status(200).json({ success: true, data: slots });

    } catch (error) {
        console.error("ViewChargingStationSlots.js:", error);
        return res.status(500).json({ success: false, message: "Internal server error!" });
    }
}

module.exports = { ViewChargingStationSlots };
